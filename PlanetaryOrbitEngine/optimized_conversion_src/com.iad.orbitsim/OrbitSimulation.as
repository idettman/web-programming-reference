package com.iad.orbitsim
{
	public class OrbitSimulation
	{
		...
		public var polyAccel:Vector.<Point3D>;
		public var polyPosition:Vector.<Point3D>;
		...
		public function OrbitSimulation (planetaryBodies:Vector.<PlanetaryBody>, timestepIncrement:Number = 1, substepsPerIteration:int = 5)
		{
			...
			this.poly = new BVector (PlanetaryBody.dejitter.length);
			this.polyIn = new BVector (PlanetaryBody.dejitter.length);
			this.polyAccel = new Vector.<Point3D> (PlanetaryBody.dejitter.length);
			this.polyPosition = new Vector.<Point3D> (PlanetaryBody.dejitter.length + 2);


			var i:int;

			for (i = 0; i < PlanetaryBody.dejitter.length; ++i)
			{
				this.polyAccel[i] = new Point3D ();
				this.polyPosition[i + 2] = new Point3D ();
			}
			polyPosition[0] = new Point3D ();
			polyPosition[1] = new Point3D ();
		...
		private function removeJitter (rescale:Number):void
		{
			for (var i:int = 0; i < planetaryBodies.length; ++i)
			{
				_planetaryBody = planetaryBodies[i];

				_point3d.minus (_planetaryBody.oldVelocity[_planetaryBody.head], _planetaryBody.oldVelocity[_planetaryBody.head + 1]);
				_point3d.plusMultiplied (_point3d, -timestepIncrement * timestepIncrement, _planetaryBody.oldAcceleration[_planetaryBody.head + 1]);

				var dp:Number = _point3d.dotProduct (_point3d);
				var da:Number = _planetaryBody.oldAcceleration[_planetaryBody.head + 1].dotProduct (_planetaryBody.oldAcceleration[_planetaryBody.head + 1]);

				if (dp > da * timestepIncrement * timestepIncrement / 256 && da != 0.0)
				{
					// trace("dejitter "+q.id+" "+dp/(da*inc*inc)+" "+ q.ov[q.head]+" "+q.ov[q.head+1]);
					_planetaryBody.dejitter (rescale, timestepIncrement, polyIn, poly, polyAccel, polyPosition);
				}
			}
		}
