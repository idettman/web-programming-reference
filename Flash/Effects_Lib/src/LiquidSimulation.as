// forked from owenray's liquid simulation (improved)
...
package
{
	...
	public class LiquidSimulation extends Sprite
	{
		...
		private var waterColor:BitmapData;
		...
		public function LiquidSimulation()
		{
		...
		public function init(event:Event):void
		{
			...
			waterColor = new BitmapData(400, 400, false, 0xFFFFFF);

			_bitmap = addChild(new Bitmap(waterColor)) as Bitmap;
		...
		public function paint(event:Event):void
		{
			...
			waterColor.fillRect(_canvas.rect,0x000080);

			for each (var p:Particle in particles)
				_canvas.setPixel(int(4.0 * p.x), int(4.0 * p.y), 0);


			_canvas.draw(_s);
			_canvas.applyFilter(_canvas, _canvas.rect, new Point(0,0), _blur);
			waterColor.threshold(_canvas,_canvas.rect,new Point(0,0), '>=', 0xF9F9F9, 0xFFCCCCCC, 0xFFFFFF, false);
